...
declare -A concepts=(
    ...
    ["Chameleon"]="chameleon"
    ["Monarch_Butterfly"]="monarch"
)

for display_name in "${!concepts[@]}"; do
    dir="${concepts[$display_name]}"
    ...
    # Step 2: Concatenate music (scene1 + scene3)
    echo "   🎵 Concatenating music..."
    echo "file 'music/scene1.mp3'" > music_concat.txt
    echo "file 'music/scene3.mp3'" >> music_concat.txt
    
    ffmpeg -y -f concat -safe 0 -i music_concat.txt -c copy music/combined_16s.mp3 2>/dev/null
    echo "      ✅ Music combined"
